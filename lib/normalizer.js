...
const Normalizer = {
  ...
  copay(text) {
    ...
    // Flat per-stay format: "$2,015 copay per stay"
    const perStayFlat = t.match(/\$([\d,]+)\s*(?:copay\s*)?per\s*stay/i);
    if (perStayFlat) {
      return `$${perStayFlat[1]} per stay`;
    }
    ...
    // Reversed format: "Days 1-5: $295 copay" (BCBSNC) or "Days 1-5: | $295" (pipe-separated)
    const reversedTiered = [...t.matchAll(/days?\s*(\d+)\s*[-–]\s*(\d+)\s*:?\s*\|?\s*\$([\d,]+(?:\.\d+)?)\s*(?:copay|per\s*day)?/gi)];
    if (reversedTiered.length > 0) {
      const parts = reversedTiered.map(m => `$${m[3].replace(/\.00$/, '')}/day, days ${m[1]}-${m[2]}`);
      return parts.join('; ');
    }
  ...
  dollar(text) {
    ...
    if (/no\s*charge|\$0(?:\s|$|\.00)|^no\s*deductible$/i.test(t) && !t.includes('-')) {
      return '$0';
    }
    // "No deductible" in context
    if (/\bno\s+deductible\b/i.test(t) && !t.match(/\$[1-9]/)) {
      return '$0';
    }
  ...
  dentalAllowance(text) {
    ...
    const annualMatch = t.match(/\$([\d,]+)\s*(?:annual|per\s*year|yearly)\s*(?:allowance|maximum|max|benefit|limit)/i)
      ...
      || t.match(/\$([\d,]+)\s*allowance\s*(?:for|every)/i);
  ...
  visionAllowance(text) {
    ...
    const eyewearM = t.match(/\$([\d,]+)\s*(?:annual\s*)?(?:eyewear|frames?|contacts?|lens(?:es)?|eyeglasses)\s*(?:allowance|benefit)?/i)
      || t.match(/(?:eyewear|frames?|contacts?|eyeglasses)\s*(?:allowance|benefit)\s*(?:of\s*)?\$([\d,]+)/i)
      || t.match(/\$([\d,]+)\s*allowance\s*(?:every\s*\d+\s*years?|per\s*year|annual)/i)
      || t.match(/\$([\d,]+)\s*(?:yearly|annual|combined)\s*(?:allowance|benefit)/i)
      || t.match(/\$([\d,]+)\s*(?:for\s*)?(?:eyeglasses|contact\s*lenses?)/i)
      || t.match(/(?:up\s*to\s*)?\$([\d,]+)\s*per\s*year/i)
      || t.match(/\$([\d,]+)\s*(?:\(combined[^)]*\))?\s*(?:vision\s*)?allowance/i);

    if (eyewearM) {
      const amt = eyewearM[1] || eyewearM[2];
      // Check for "every X years" period
      const everyYears = t.match(/every\s*(\d+)\s*years?/i);
      const periodStr = everyYears ? ` every ${everyYears[1]} years` : '';
      // Look for exam copay
      const examCopay = t.match(/\$(\d+)\s*(?:copay\s*)?(?:for\s*)?(?:routine\s*)?exam/i);
      if (examCopay) {
        return `$${amt} eyewear allowance${periodStr} / $${examCopay[1]} exam copay`;
      }
      return `$${amt} eyewear allowance${periodStr}`;
    }
  ...
  transportation(text) {
    ...
    // Trip count patterns
    const trips = t.match(/(\d+)\s*(?:one[- ]?way|round[- ]?trip)?\s*trips?\s*(?:per\s*year|every\s*year|annual|yearly)?/i);
