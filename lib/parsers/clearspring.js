...
ClearSpringParser.extractSpecialist = function(result, allPages, prose) {
  // ClearSpring: "Specialist: $0 to $35 copay"
  const m = prose.match(/Specialist\s*:?\s*\$([\d,]+)\s*(?:to|-)\s*\$([\d,]+)\s*copay/i);
  if (m) { result.specialistCopay = `$${m[1]} - $${m[2]} copay`; return; }
  const m2 = prose.match(/Specialist\s*:?\s*\$([\d,]+)\s*copay/i);
  if (m2) { result.specialistCopay = `$${m2[1]} copay`; return; }
