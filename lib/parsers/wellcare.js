...
WellcareParser.extractPlanName = function(result, allPages, prose) {
  // Try to find specific plan name (Giveback/Simple/Assist)
  const m = prose.match(/(Wellcare\s+(?:Giveback|Simple|Assist)\s+(?:Open\s*)?\([^)]+\))/i);
  // Look for "H2775, Plan 111" format or "H2775 | 111 | 000"
  const hM = prose.match(/(H\d{4})\s*[|,]\s*(?:Plan\s*)?(\d{3})/i)
    || prose.match(/(H\d{4})\s*[-_]\s*(\d{3})/i);
...
WellcareParser.extractDental = function(result, allPages, prose) {
  // Wellcare Giveback: "dental services with no annual maximum allowance"
  if (/no\s*annual\s*(?:limit|cap|maximum)[^.]{0,40}(?:dental|allowance)/i.test(prose) ||
      /dental[^.]{0,60}no\s*annual\s*(?:limit|cap|maximum)/i.test(prose)) {
