...
MolinaParser.extractPremium = function(result, allPages, prose) {
  // Molina: "Monthly Plan Premium $0" — may not be in PDF.js text
  const m = prose.match(/Monthly\s*Plan\s*Premium\s*\$([\d,.]+)/i)
    || prose.match(/Premium[^$]{0,20}\$([\d,.]+)/i);
  if (m) { result.planPremium = `$${m[1].replace(/\.00$/, '')}/month`; return; }
  // Check rawText from table-extract
  const raw = this.getRawText(allPages);
  const m2 = raw.match(/Premium[^$]{0,20}\$([\d,.]+)/i)
    || raw.match(/\$([\d,.]+)\s*(?:per\s*month|\/month)/i);
  if (m2) { result.planPremium = `$${m2[1].replace(/\.00$/, '')}/month`; return; }
  // Molina plans are typically $0
