...
CloverParser.extractPremium = function(result, allPages, prose) {
  if (/\$0\s*per\s*month/i.test(prose)) { result.planPremium = '$0/month'; return; }
...
CloverParser.extractVision = function(result, allPages, prose) {
  // "$300 combined (in-and out-of-network) yearly allowance per plan year"
  const m = prose.match(/\$([\d,]+)\s*combined\s*[^.]{0,40}(?:yearly|annual)\s*(?:vision\s*)?allowance/i)
    || prose.match(/\$([\d,]+)\s*(?:yearly|annual)\s*(?:vision\s*)?(?:allowance|benefit)/i)
