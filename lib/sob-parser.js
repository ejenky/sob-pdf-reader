...
const SOBParser = {
  ...
  /**
   * Parse structured page data into benefit fields.
   * If carrier-specific parsers are available, dispatches to them.
   * Falls back to generic parsing if no carrier parser found.
   * @param {Array} allPages - Array of { rows, rawText, proseText } from table-extract
   * @param {Array} pagesRaw - Raw page items (optional, for carrier detection)
   * @param {string} carrierHint - Carrier name hint (optional)
   * @returns {Object} Extracted benefit fields
   */
  parse(allPages, pagesRaw, carrierHint) {
    // Try carrier-specific parsing first
    const carrier = carrierHint || this._detectCarrier(allPages, pagesRaw);
    const PARSERS = {
      aetna: typeof AetnaParser !== 'undefined' ? AetnaParser : null,
      uhc: typeof UHCParser !== 'undefined' ? UHCParser : null,
      anthem: typeof AnthemParser !== 'undefined' ? AnthemParser : null,
      healthspring: typeof HealthSpringParser !== 'undefined' ? HealthSpringParser : null,
      clearspring: typeof ClearSpringParser !== 'undefined' ? ClearSpringParser : null,
      zing: typeof ZingParser !== 'undefined' ? ZingParser : null,
      clover: typeof CloverParser !== 'undefined' ? CloverParser : null,
      kaiser: typeof KaiserParser !== 'undefined' ? KaiserParser : null,
      molina: typeof MolinaParser !== 'undefined' ? MolinaParser : null,
      bcbsnc: typeof BCBSNCParser !== 'undefined' ? BCBSNCParser : null,
      wellcare: typeof WellcareParser !== 'undefined' ? WellcareParser : null,
    };
    const parser = PARSERS[carrier];
    if (parser) {
      try {
        return parser.parse(allPages, pagesRaw);
      } catch(e) {
        console.warn(`Carrier parser ${carrier} failed:`, e, '— falling back to generic');
      }
    }
    // Fall back to existing generic parsing
    return this._parseGeneric(allPages);
  },

  _detectCarrier(allPages, pagesRaw) {
    if (typeof CarrierDetect !== 'undefined') {
      if (pagesRaw) return CarrierDetect.detect(pagesRaw).carrier;
      // Try from proseText
      const prose = allPages.map(p => p.proseText || '').join(' ');
      return CarrierDetect.detectFromText(prose).carrier;
    }
    return 'generic';
  },

  _parseGeneric(allPages) {
    const result = {};
    ...
    // Fix specialist — if same as PCP, it's wrong (grabbed PCP row value)
    if (result.specialistCopay === 'Not found' || result.specialistCopay === result.pcpCopay) {
      const specM = allProse.match(/Specialist[s]?\s*\$([\d,]+)\s*(?:copay|per\s*visit)/i)
        || allProse.match(/Specialist[^$]{0,30}\$([\d,]+)\s*(?:copay|per\s*visit)/i)
        || allProse.match(/Specialist[^$]{0,30}You\s*pay\s*\$([\d,]+)/i);
      if (specM) {
        const specVal = `$${specM[1]} copay`;
        if (specVal !== result.pcpCopay) result.specialistCopay = specVal;
      }
    }

    // Fix premium — clean up trailing periods ($0./month → $0/month)
    if (result.planPremium) result.planPremium = result.planPremium.replace(/\.\s*\/month/, '/month');

    // Fix premium Not found — search prose
    if (result.planPremium === 'Not found') {
      if (/\$0\s*(?:Plan\s*)?Premium|\$0\.\s*You\s*must\s*continue/i.test(allProse)) {
        result.planPremium = '$0/month';
      } else {
        const premFB = allProse.match(/Premium[^$]{0,30}\$([\d,.]+)/i);
        if (premFB) result.planPremium = `$${premFB[1].replace(/\.00$/, '')}/month`;
      }
    }

    // Preventive care — ALWAYS $0 copay on Medicare Advantage
    if (result.preventiveCare === 'Not found' || !/^\$0/.test(result.preventiveCare)) {
      result.preventiveCare = '$0 copay';
    }

    // Fix med deductible Not found — search prose for $0 or "no deductible"
    if (result.medDeductible === 'Not found') {
      if (/(?:medical|plan)\s*(?:deductible)[^$]{0,30}\$0/i.test(allProse) ||
          /Deductible\s*\(medical\)[^$]{0,20}\$0/i.test(allProse) ||
          /no\s*deductible\s*(?:for\s*)?medical/i.test(allProse) ||
          /\$0\.00[^.]{0,30}(?:medical|plan)\s*deductible/i.test(allProse)) {
        result.medDeductible = '$0';
      }
    }

    // Fix food card — sanitize junk text (> 30 chars without $)
    if (result.foodFlexCard && result.foodFlexCard.length > 30 && !/^\$/.test(result.foodFlexCard)) {
      result.foodFlexCard = '$0';
    }

    // Fix UHC plan name — "Summary of H..." → find full name
    if (/^Summary\s*of\s*H/i.test(result.planName)) {
      const uhcName = allProse.match(/(UHC\s+[A-Z][^(]{3,40}\([^)]+\))/i)
        || allProse.match(/(UnitedHealthcare\s+[A-Z][^(]{3,40}\([^)]+\))/i);
      if (uhcName) {
        const hNum = result.planName.match(/H\d{4}[-_]\d{3}/);
        result.planName = uhcName[1].trim() + (hNum ? ` ${hNum[0]}` : '');
      }
    }

    // Fix Clover Part B — "$20 subsidy"
    if ((result.partBReduction === 'N/A' || result.partBReduction === 'Not found') &&
        /subsidy|buy[- ]?down|credit/i.test(allProse)) {
      const partBBuy = allProse.match(/\$([\d,.]+)\s*(?:subsidy|buy[- ]?down|credit)[^.]{0,40}Part\s*B/i)
        || allProse.match(/Part\s*B[^$]{0,40}\$([\d,.]+)\s*(?:subsidy|buy[- ]?down|credit)/i);
      if (partBBuy) result.partBReduction = `$${partBBuy[1].replace(/\.00$/, '')}/month`;
    }

    // Fix vision Not found — search prose
    if (result.visionAllowance === 'Not found') {
      const visFB = allProse.match(/\$([\d,]+)\s*(?:annual\s*)?(?:benefit\s*)?(?:allowance)[^.]{0,40}(?:eyewear|eyeglasses|frames|contacts|prescription)/i)
        || allProse.match(/(?:eyewear|frames|contacts)\s*(?:allowance|benefit)[^$]{0,40}\$([\d,]+)/i);
      if (visFB) result.visionAllowance = `$${visFB[1] || visFB[2]} eyewear allowance`;
    }

    // Fix hospital — if only first tier, search prose for next tier
    if (result.hospitalCopay && /^\$\d.*days\s*1-\d+$/.test(result.hospitalCopay) && !/;/.test(result.hospitalCopay)) {
      const firstEnd = result.hospitalCopay.match(/days\s*1-(\d+)/);
      if (firstEnd) {
        const nextDay = parseInt(firstEnd[1]) + 1;
        const re = new RegExp(`\\$0\\s*(?:copay\\s*)?per\\s*day[^.]{0,30}days?\\s*${nextDay}|days?\\s*${nextDay}\\s*(?:and\\s*beyond|[-]\\s*\\d+)[^.]{0,30}\\$0|You\\s*pay\\s*\\$0[^.]{0,20}days?\\s*${nextDay}`, 'i');
        const nextM = allProse.match(re);
        if (nextM) {
          const endM = nextM[0].match(/days?\s*\d+\s*[-–]\s*(\d+)/);
          result.hospitalCopay += `; $0/day, days ${nextDay}${endM ? `-${endM[1]}` : '+'}`;
        }
      }
    }

    // Fix Healthspring hearing — copay range for aids
    if (result.hearingAllowance === '$0' || result.hearingAllowance === 'Not found') {
      const hearRange = allProse.match(/\$([\d,]+)\s*[-–]\s*\$([\d,]+)\s*(?:copay|per)[^.]{0,40}hearing\s*aid/i)
        || allProse.match(/hearing\s*aid[^.]{0,80}\$([\d,]+)\s*[-–]\s*\$([\d,]+)\s*copay/i);
      if (hearRange) result.hearingAllowance = `$${hearRange[1]} - $${hearRange[2]} copay per hearing aid`;
    }

    // Fix BCBSNC/Clover hearing — TruHearing copay tiers
    if (result.hearingAllowance === 'Not found') {
      const truHear = allProse.match(/\$([\d,]+)[^.]{0,20}(?:Standard|Select|Basic)[^.]{0,40}\$([\d,]+)[^.]{0,20}(?:Advanced|Premium|Plus)/i);
      if (truHear) result.hearingAllowance = `$${truHear[1]} - $${truHear[2]} copay per hearing aid`;
    }

    // Fix BCBSNC/Molina transportation — rides
    if (result.transportation === '$0') {
      const rideM = allProse.match(/(\d+)\s*(?:one[- ]?way\s*)?(?:rides?|trips?)\s*(?:per\s*(?:year|calendar)|every\s*year|annually)/i);
      if (rideM) result.transportation = `${rideM[1]} rides/year ($0 copay)`;
    }

    // Fix Molina hearing — small copay is exam not hearing aid
    if (result.hearingAllowance && /^\$\d+\s/.test(result.hearingAllowance)) {
      ...
      if (hVal.length > 0 && hVal[0].value > 0 && hVal[0].value < 100) {
        ...
        result.hearingAllowance = hDesc ? `${hDesc[1]} hearing aids every ${hDesc[2]} years` : 'See plan details';
      }
    }
