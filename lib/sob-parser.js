  ...
  refineMOOP(result, allPages) {
    ...
    for (const page of allPages) {
      for (let i = 0; i < page.rows.length; i++) {
        ...
        // Match "MOOP" as a standalone short label (not inside section titles like
        // "Plan premium, deductible, and maximum out-of-pocket (MOOP)")
        const isShortMoopLabel = /^\s*MOOP\s*$/i.test(label);
        const isMoopKeyword = /\bMOOP\b/i.test(label) && label.length < 30;
        const isMaxOOP = /maximum\s*out/i.test(label) && label.length < 50;
        if (!isShortMoopLabel && !isMoopKeyword && !isMaxOOP) continue;
