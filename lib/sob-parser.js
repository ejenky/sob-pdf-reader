...
const SOBParser = {
  ...
  refineMOOPAndDeductible(result, allPages) {
    for (const page of allPages) {
      for (let i = 0; i < page.rows.length; i++) {
        ...
        // Plan deductible: label starts with "Plan deductible"
        if (/^Plan\s*deductible/i.test(label)) {
          const value = row.inNetwork || '';
          const dollars = Normalizer.extractDollars(value);
          if (dollars.length > 0) {
            result.medDeductible = `$${dollars[0].raw}`;
          } else if (/\$0/i.test(value) || /\$0/.test(label)) {
  ...
  refineHospital(result, allPages) {
    // Use rawText from each page to find Inpatient tiers.
    // rawText is built from merged rows as "label | inNetwork | outOfNetwork"
    // per row, joined by newlines. This avoids column detection issues.
    for (const page of allPages) {
      const raw = (page.rawText || '').replace(/[\u2010-\u2015\u2212]/g, '-');

      // Find "Inpatient" in the page raw text
      const idx = raw.search(/\bInpatient\b(?!.*psychiatric)/i);
      if (idx === -1) continue;

      // Get text from Inpatient up to Outpatient (or 800 chars)
      ...
      // Extract ALL per-day tiers
      const allTiers = [...segment.matchAll(/\$([\d,]+)\s*(?:copay\s*)?per\s*day,?\s*days?\s*(\d+)\s*[-–]\s*(\d+)/gi)];

      if (allTiers.length > 0) {
        // Sequential day logic: start with first tier (days 1-X),
        // then find tier starting at X+1
        const inNetTiers = [allTiers[0]];
        let nextDayStart = parseInt(allTiers[0][3]) + 1;

        for (const tier of allTiers.slice(1)) {
          if (parseInt(tier[2]) === nextDayStart) {
