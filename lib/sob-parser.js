...
const SOBParser = {
  ...
  refineMOOP(result, allPages) {
    ...
    for (const page of allPages) {
      for (let i = 0; i < page.rows.length; i++) {
        ...
        if (!/\bMOOP\b|maximum\s*out|out.*pocket.*max|most.*you.*pay/i.test(label)) continue;
  ...
  /**
   * Refine hospital inpatient — gather per-day tiers from the row AND its continuation rows.
   * The label "Inpatient (unlimited number of days)" spans 2 PDF rows, and the continuation
   * row has "days 7-90; $0 for additional days" that we need.
   */
  refineHospital(result, allPages) {
    for (const page of allPages) {
      for (let i = 0; i < page.rows.length; i++) {
        ...
        // Gather in-network text from this row AND next few rows
        // (continuation rows may have "days 7-90" data)
        let fullInNetwork = row.inNetwork || '';
        for (let j = i + 1; j < Math.min(i + 4, page.rows.length); j++) {
          const nextRow = page.rows[j];
          const nextLabel = (nextRow.label || '').trim();
          // Stop if we hit a new benefit label (like "Outpatient hospital")
          if (nextLabel && /^[A-Z]/.test(nextLabel) && nextLabel.length > 15) break;
          if (/outpatient|ambulatory|observation/i.test(nextLabel)) break;
          // Append in-network data from continuation
          if (nextRow.inNetwork) {
            fullInNetwork += ' ' + nextRow.inNetwork;
          }
        }

        fullInNetwork = fullInNetwork.replace(/[\u2010-\u2015\u2212]/g, '-').replace(/\s+/g, ' ').trim();

        // Extract per-day tiers
        const tiers = [...fullInNetwork.matchAll(/\$([\d,]+)\s*(?:copay\s*)?per\s*day,?\s*days?\s*(\d+)\s*[-–]\s*(\d+)/gi)];
        if (tiers.length > 0) {
          // Filter out OON tiers: if two tiers start at day 1, keep only the first group
          const day1Tiers = tiers.filter(m => m[2] === '1');
          let finalTiers = tiers;
          if (day1Tiers.length >= 2) {
            const secondDay1Idx = tiers.indexOf(day1Tiers[1]);
            finalTiers = tiers.slice(0, secondDay1Idx);
          }
          result.hospitalCopay = finalTiers.map(m => `$${m[1]}/day, days ${m[2]}-${m[3]}`).join('; ');
        } else if (/\$0\s*copay/i.test(fullInNetwork)) {
