...
const SOBParser = {
      ...
      for (let i = 0; i < page.rows.length; i++) {
        ...
        if (result.erCopay === 'Not found' && /^Emergency\b/i.test(label) && !/urgent/i.test(label)) {
          ...
          if (!erVal.match(/\$([\d,]+)\s*copay/i) && i > 0) {
            ...
            const prevLabel = (prevRow.label || '').trim();
            if (!prevLabel || prevLabel.length < 5) {
              erVal = prevRow.inNetwork || '';
            }
          }
          // Also gather next few rows
          for (let j = i + 1; j < Math.min(i + 4, page.rows.length); j++) {
            const nextRow = page.rows[j];
            const nextLabel = (nextRow.label || '').trim();
            if (/urgent/i.test(nextLabel)) break;
            if (!nextLabel || nextLabel.length < 5) {
              erVal += ' ' + (nextRow.inNetwork || '');
            }
          }
  ...
  parseFlowingText(result, prose, allPages) {
    ...
    if (result.pcpCopay === 'Not found') {
      const pcpSeg = findAfterInText(t, 'Primary care physician.*visit', 300)
    ...
    if (result.hospitalCopay === 'Not found' || result.hospitalCopay === '$0/day, days 1-6') {
    ...
    if (result.otcAllowance === 'Not found' || (result.otcAllowance && parseInt(result.otcAllowance.replace(/[^0-9]/g, '')) > 500)) {
      const otcSeg = findAfterInText(t, 'Over-the-Counter', 500);
      if (otcSeg) {
    ...
    if (result.dentalAllowance === 'Not found' || !/^\$[\d,]+/.test(result.dentalAllowance)) {
      const dentalSeg = findAfterInText(t, 'Dental Services', 1000)
    ...
    // Transportation: "Transportation Not Covered"
    if (result.transportation === 'Not found') {
      const transSeg = findAfterInText(t, 'Transportation', 200);
      if (transSeg) {
        if (/Not\s*Covered/i.test(transSeg.substring(0, 80))) {
          result.transportation = 'Not covered';
        } else {
