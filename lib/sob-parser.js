  ...
  refineHospital(result, allPages) {
    for (const page of allPages) {
      for (let i = 0; i < page.rows.length; i++) {
        ...
        // Gather ONLY in-network text from this row and next few rows.
        // Must keep in-network separate from out-of-network to avoid
        // OON text breaking the regex match between "$0 per day," and "days 7-90"
        let inNetText = '';
        for (let j = i; j < Math.min(i + 5, page.rows.length); j++) {
          ...
          if (r.inNetwork) inNetText += ' ' + r.inNetwork;
        }
        inNetText = inNetText.replace(/[\u2010-\u2015\u2212]/g, '-').replace(/\s+/g, ' ').trim();

        // Extract per-day tiers from in-network text only
        const allTiers = [...inNetText.matchAll(/\$([\d,]+)\s*(?:copay\s*)?per\s*day,?\s*days?\s*(\d+)\s*[-–]\s*(\d+)/gi)];
