...
const SOBParser = {
  ...
  FIELD_DEFS: [
    ...
    },
    {
      ...
      antiPatterns: [/dental/i, /hearing/i, /diagnostic/i, /glaucoma/i, /cataract/i],
      source: 'inNetwork',
  ...
  parse(allPages) {
    ...
    if (result.rxDeductible === 'Not found') {
      ...
      if (rxM) {
        result.rxDeductible = `$${rxM[1] || rxM[2]}`;
      }
    }
  ...
  normalizeField(def, value, row) {
    ...
    switch (norm) {
      ...
      case 'medDeductibleSpecial': {
        ...
        if (/does\s*not\s*have|no\s*(?:medical\s*)?deductible/i.test(t)) return '$0';
  ...
  refineDeductibles(result, allPages, fullText) {
    // Medical deductible: handle "does not have a medical deductible" text
    if (result.medDeductible !== 'Not found' && /does\s*not\s*have|no\s*(?:medical\s*)?deductible/i.test(result.medDeductible)) {
    ...
    if (result.medDeductible === 'Not found') {
      const dedSeg = findAfterInText(t, 'medical deductible', 300)
        || findAfterInText(t, 'annual medical deductible', 300);
      if (dedSeg) {
        if (/does\s*not\s*have|no\s*(?:medical\s*)?deductible|\$0/i.test(dedSeg)) {
          result.medDeductible = '$0';
        } else {
  ...
  refineEmergencyUrgent(result, allPages, fullText) {
    ...
    if (result.erCopay === 'Not found' || result.erCopay === '$0') {
      // Search for "Emergency care" label then grab the FIRST dollar amount after it
    ...
    if (result.urgentCopay === 'Not found' || result.urgentCopay === '$0') {
      // UHC: "Urgently needed services $50 copay"
  ...
  refineMOOPAndDeductible(result, allPages) {
    for (const page of allPages) {
      for (let i = 0; i < page.rows.length; i++) {
        ...
        // Plan deductible / Annual medical deductible
        if (/^Plan\s*deductible/i.test(label) || /^Annual\s*medical\s*deductible/i.test(label)) {
          const value = row.inNetwork || label;
          // "This plan does not have a medical deductible"
          if (/does\s*not\s*have|no\s*(?:medical\s*)?deductible/i.test(value)) {
            result.medDeductible = '$0';
          } else {
            const dollars = Normalizer.extractDollars(value);
            if (dollars.length > 0) {
