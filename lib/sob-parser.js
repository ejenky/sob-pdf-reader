...
const SOBParser = {
  ...
  parse(allPages) {
    ...
    // Pass 10: Link OTC and Food/Flex for Aetna Extra Supports Wallet plans
    if (result.otcAllowance && /extra\s*supports?\s*wallet/i.test(result.otcAllowance)) {
      // Extract the dollar amount from OTC
      const m = result.otcAllowance.match(/\$([\d,]+)\/month/);
      if (m) {
        result.otcAllowance = `$${m[1]}/month (Extra Supports Wallet) Food If Applicable`;
        result.foodFlexCard = `$${m[1]}/month (Extra Supports Wallet) Food If Applicable`;
      }
    }
  ...
  refineHospital(result, allPages) {
    ...
    for (const page of allPages) {
      ...
      // Find all "$X per day" amounts and all "days N-M" ranges separately
      ...
      if (perDayAmounts.length > 0 && dayRanges.length > 0) {
        ...
        if (tiers.length > 0) {
          ...
          result.hospitalCopay = inNetTiers.map(t => `$${t.amount}/day, days ${t.start}-${t.end}`).join('; ');
          return;
        }
      }
